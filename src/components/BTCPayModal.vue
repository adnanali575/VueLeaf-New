  ...
  <div class="fixed inset-0 z-[9999]">
    ...
    <div class="fixed inset-0 overflow-hidden">
      <div class="flex min-h-full items-center justify-center p-6">
        <div
          class="relative w-full max-w-[600px] bg-white flex flex-col h-[800px] max-h-[90vh] overflow-hidden rounded-[20px] shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1),0_10px_10px_-5px_rgba(0,0,0,0.04)] ring-1 ring-gray-200/50">
          ...
          <div class="relative flex-1 overflow-hidden bg-white">
            <template v-if="props.loading">
              <div class="flex items-center justify-center min-h-[600px]">
                <div class="relative w-32 h-32">
                  <!-- Outer spinning ring -->
                  <div
                    class="absolute inset-0 rounded-full border-[10px] border-t-green-400 border-r-green-500 border-b-green-600 border-l-green-300 animate-[spin_1s_linear_infinite]">
                  </div>
                  <!-- Inner gradient circle -->
                  <div
                    class="absolute inset-6 rounded-full bg-gradient-to-br from-green-300 via-green-500 to-green-600 animate-pulse">
                  </div>
                  <!-- Loading text -->
                  <div
                    class="absolute -bottom-10 left-1/2 -translate-x-1/2 whitespace-nowrap text-base font-medium text-gray-600">
                    Creating Invoice...
                  </div>
                </div>
              </div>
            ...
            <template v-else>
              <iframe :src="props.invoiceUrl" class="absolute inset-0 w-full h-full border-0" allow="payment"
