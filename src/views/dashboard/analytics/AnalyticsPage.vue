  ...
  <div class="flex-1 p-2 md:p-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 md:py-8 space-y-6">
      ...
      <div v-if="analytics.error" class="text-center py-8">
        ...
        <button @click="analytics.fetchAnalyticsData"
      ...
      <template v-else>
        ...
        <div class="mb-8 bg-white rounded-lg shadow-sm p-4 sm:p-6">
          <div class="flex flex-col md:flex-row items-center gap-6">
            <div class="flex-1 text-center md:text-left">
              ...
              <p v-else class="text-gray-600 text-sm sm:text-md md:text-lg">
                Ready to explore your brand's impact? Here's a complete view of
                what people are saying about you across the growing community.
                Dive into your latest analytics to understand how your brand
                resonates with growers and identify opportunities for growth. We
                track and analyze conversations across all major growing
                platforms to give you comprehensive insights about your brand's
            ...
            <div class="w-full max-w-xs sm:max-w-sm md:w-48 md:h-48 flex-shrink-0 relative">
              ...
              <img v-else :src="analyticsImage" alt="Analytics illustration" class="w-full h-full object-contain" />
            </div>
          </div>
        </div>
        ...
        <div class="mb-8">
          <div class="grid grid-cols-2 grid-rows-2 sm:grid-rows-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-if="analytics.isLoading" v-for="i in 4" :key="i"
            ...
            <template v-else>
              <div v-for="(card, index) in statCards" :key="index"
                ...
                <div class="flex items-center justify-between mb-4">
                  <h3 :class="[card.color, 'text-sm font-medium']">
                    {{ card.title }}
                  </h3>
                  <component :is="card.icon" :class="[card.color, 'h-4 w-4']" />
                </div>
                <div>
                  <div :class="[card.color, 'text-3xl font-bold']">
                    {{ card.value }}
                  </div>
                  <p :class="[
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between gap-2 flex-wrap sm:flex-nowrap mb-4">
            <div class="flex items-center gap-3 w-full sm:w-auto text-center sm:text-left">
              <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-gray-100">
                <HeartPulse class="w-5 h-5 aspect-square text-gray-500" />
              </div>
              <div>
                ...
                <p class="text-base text-gray-500">
                  Trend of positive, negative and neutral mentions
                </p>
              </div>
            </div>
          ...
          <div v-else-if="!analytics.sentimentOverTime?.length"
            ...
            <p class="text-gray-500 text-sm mb-5 max-w-[700px]">
              Track how brand sentiment evolves over time. Watch trends and
              respond to customer perceptions.
            </p>
          </div>
          <SentimentChart v-else :data="analytics.sentimentOverTime"
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between gap-2 flex-wrap sm:flex-nowrap mb-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-base text-gray-500">
                  Analyze which keywords are getting the most traction and
                  where.
                </p>
              </div>
            </div>
            <p v-if="analytics.filters.minMentions > 0" class="text-sm text-gray-600 max-sm:mx-auto">
          ...
          <div v-else-if="!analytics.keywordAnalysis?.length"
            class="flex flex-col items-center justify-center h-64 p-6 text-center">
            ...
            <p class="text-gray-500 text-sm mb-5 max-w-[700px]">
              Discover which keywords drive engagement. Track performance
              metrics and optimize brand monitoring.
            </p>
          </div>
          ...
          <div class="flex items-center justify-between gap-2 flex-wrap sm:flex-nowrap mb-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-base text-gray-500">
                  A visualization of mentions by platform.
                </p>
              </div>
            </div>
            <div class="text-sm max-sm:m-auto text-gray-600">
              Sorted by {{ analytics.filters.sortBy }}
            </div>
          </div>
          <div v-if="analytics.isLoading" class="h-[300px] bg-gray-50 rounded animate-pulse"></div>
          <div v-else-if="!analytics.platformSummary?.length"
            class="flex flex-col items-center justify-center h-64 p-6 text-center">
            ...
            <p class="text-gray-500 text-sm mb-5 max-w-[700px]">
              Get insights across cannabis community platforms. Connect your
              first platform to track performance.
            </p>
          </div>
          ...
          <div class="flex items-center justify-between gap-2 flex-wrap sm:flex-nowrap mb-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-base text-gray-500">
                  Analyze how keywords correlate with sentiment.
                </p>
              </div>
            </div>
            <div class="text-sm max-sm:m-auto text-gray-600">
              {{
                analytics.filters.platforms.length
                  ? analytics.filters.platforms.join(", ")
                  : "All Platforms"
              }}
            </div>
          </div>
          <div v-if="analytics.isLoading" class="h-[300px] bg-gray-50 rounded animate-pulse"></div>
          <div v-else-if="!analytics.keywordCorrelation?.length"
            class="flex flex-col items-center justify-center h-64 p-6 text-center">
            ...
            <p class="text-gray-500 text-sm mb-5 max-w-[700px]">
              Discover relationships between keywords and topics. Add keywords
              to reveal brand patterns.
            </p>
          </div>
