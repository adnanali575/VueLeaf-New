  ...
  <div class="grid grid-cols-2 sm:grid-rows-1 grid-rows-2 gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
    ...
    <template v-else>
      <div v-for="stat in stats" :key="stat.title"
        ...
        <Card class="h-full" :class="stat.title === 'Total Keywords' ? 'shadow-sm hover:shadow-md transition-all duration-200 border border-blue-500/10 bg-gradient-to-br from-blue-100/60 to-indigo-100/60' :
          ...
          ">
          <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle :class="stat.title === 'Total Keywords' ? 'text-sm font-medium text-blue-700' :
            ...
            <component :is="stat.icon" :class="stat.title === 'Total Keywords' ? 'h-4 w-4 text-blue-700' :
          ...
          <CardContent>
            <div :class="stat.title === 'Total Keywords' ? 'text-3xl font-bold text-blue-700' :
            ...
            <template v-if="stat.subtext">
              <p :class="stat.title === 'Total Keywords' ? 'mt-2 text-sm text-blue-700/80' :
            ...
            <div v-if="stat.title === 'Keyword Limit'" class="mt-2 mb-4 relative z-10">
              <div class="h-1.5 w-full rounded-full bg-gray-200">
                <div class="h-1.5 rounded-full bg-indigo-700 mt-[26px] sm:mt-[0px] transition-all duration-200"
