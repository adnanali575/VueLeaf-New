...
<template>
  <div class="flex-1 p-2 md:p-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 md:py-8 space-y-6">
      <!-- Welcome Section -->
        ...
        <div class="flex flex-col md:flex-row items-center gap-6">
          <div class="flex-1 text-center md:text-left">
            ...
            <p class="text-gray-600 text-sm sm:text-md md:text-lg">
              Keep your finger on the pulse of the market. Track how your competitors are performing across the growing
              community and understand where your brand stands. We analyze discussions, sentiment, and trends across all
              major growing platforms to give you real-time insights into market dynamics.
            </p>
          </div>
          <div class="w-full max-w-xs sm:max-w-sm md:w-44 md:h-44 flex-shrink-0 relative">
            <img :src="analyticsImage" alt="Competitor Monitoring illustration" class="w-full h-full object-contain" />
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-end mb-6">
        <Popover v-model:open="showAddPopover">
          <PopoverTrigger class="max-sm:w-full">
            <Button class="bg-green-600 hover:bg-green-700 text-white w-full">
          ...
          <PopoverContent class="w-80">
            <div class="space-y-4">
              ...
              <div>
                <Input v-model="newCompetitorName" placeholder="Enter competitor name" :disabled="addingCompetitor"
                  @keyup.enter="addCompetitor" />
                <p v-if="addError" class="mt-2 text-sm text-red-600">{{ addError }}</p>
              </div>
              <div class="flex justify-end gap-2">
                <Button variant="outline" @click="showAddPopover = false" :disabled="addingCompetitor">
                  Cancel
                </Button>
                <Button @click="addCompetitor" :disabled="addingCompetitor"
      ...
      <!-- Empty State -->
      <div v-if="!competitorStore.competitors.length && !competitorStore.isLoading"
        class="text-center p-12 bg-white rounded-lg shadow-sm">
        ...
        <p class="text-gray-500 mb-4">Add your first competitor to start monitoring their presence across growing
          communities.</p>
      ...
      <template v-else-if="competitorStore.isLoading">
        ...
        <Card class="mt-6">
          ...
          <CardContent class="pt-6">
            <div class="h-[400px] bg-gray-100 rounded-lg border border-gray-200 animate-pulse"></div>
          </CardContent>
        </Card>
      ...
      <template v-else>
        ...
        <Card>
          ...
          <CardContent class="pt-6">
            <div v-if="competitorStore.competitors.length"
              class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <Card v-for="(competitor, index) in competitorStore.competitors" :key="index"
                :class="cn(`bg-${competitor.color}-50 border-${competitor.color}-200`)">
                <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle class="text-sm font-medium">
                    {{ competitor.name }}
                    <Badge v-if="competitor.isMain"
                      :class="cn(`ml-2 bg-${competitor.color}-100 text-${competitor.color}-800`)">
                      Main
                    </Badge>
                    <Badge v-if="competitor.name === 'Your Brand'" class="ml-2 bg-green-100 text-green-800">
                      Primary
                    </Badge>
                  </CardTitle>
                  <div class="flex items-center space-x-2">
                    <Button variant="ghost" size="sm" class="text-red-600 hover:text-red-800 hover:bg-red-100"
            ...
            <div class="p-4 bg-gray-100 border border-gray-200 rounded-lg">
              <p class="text-sm text-gray-700 font-medium">
                Monitor how your brand and competitors perform across growing communities. Each card shows total
                mentions and
                overall sentiment trends, helping you track relative market presence and community perception. This
                snapshot gives
                you quick insights into how your brand stands against key players in the market.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent class="pt-6">
            <template
              v-if="!competitorStore.isLoading && !competitorStore.error && (!competitorStore.sentimentData.length || competitorStore.sentimentData.every(item => !item.positive && !item.neutral && !item.negative))">
            ...
            <div class="p-4 bg-gray-100 border border-gray-200 rounded-lg">
              <p class="text-sm text-gray-700 font-medium">
                Compare how different brands are perceived in growing community discussions. The bars show the
                proportion of
                positive (green), neutral (yellow), and negative (red) mentions for each brand, letting you benchmark
                your
                reputation against competitors. This comparison helps identify where your brand stands out and where
                there might
                be opportunities to improve relative to other players in the market.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent class="pt-6">
            <div v-if="!competitorStore.isLoading && !competitorStore.error && !competitorStore.priorityAlerts.length"
              class="flex flex-col items-center justify-center h-64 p-6 text-center">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div v-for="(alert, index) in competitorStore.priorityAlerts" :key="index"
                ...
                <div class="flex-1">
                  ...
                  <div class="flex items-center mt-2">
                    <Badge :class="cn(`mr-2 ${alert.importance === 'urgent' ? 'bg-red-100 text-red-800' :
            ...
            <div class="p-4 bg-gray-100 border border-gray-200 rounded-lg">
              <p class="text-sm text-gray-700 font-medium">
                Stay ahead of significant market developments. These alerts highlight important changes in competitor
                activities, from notable sentiment shifts to new product discussions and emerging trends. Real-time
                notifications ensure you never miss crucial developments that could impact your market position in the
                growing
                community.
              </p>
            </div>
          ...
          <CardContent class="pt-6">
            ...
            <div class="p-3 bg-gray-100 border border-gray-200 rounded-lg">
              <p class="text-xs text-gray-700 font-medium">
                Track brand mentions across key cannabis forums. Monitor trends and engagement levels to understand
                where your
                audience is most active and how discussions are evolving. This data helps you focus your efforts on the
                most
                impactful platforms.
              </p>
            </div>
        ...
        <Card>
          ...
          <CardContent class="pt-6">
            <!-- Discussion Tabs -->
            <div v-if="discussionTabs.length" class="flex flex-wrap gap-2 mb-4">
              <Button v-for="tab in discussionTabs" :key="tab.id"
            ...
            <div v-if="!competitorStore.isLoading && !competitorStore.error && !filteredDiscussions.length"
              class="flex flex-col items-center justify-center h-64 p-6 text-center">
            ...
            <div v-else-if="filteredDiscussions.length > 0" class="space-y-4">
              <Card v-for="(discussion, index) in filteredDiscussions" :key="index"
                ...
                <CardContent class="p-4">
                  <div class="flex items-center justify-between mb-2">
                    ...
                    <Badge variant="outline" :class="cn(
                      ...
                      <component :is="getSentimentIcon(discussion.sentiment)" :class="cn(
                        'mr-1 h-4 w-4',
            ...
            <div class="p-4 mt-6 bg-gray-100 border border-gray-200 rounded-lg">
              <p class="text-sm text-gray-700 font-medium">
                Stay informed with real-time discussions about your brand and competitors. Quickly gauge sentiment and
                identify
                emerging topics or concerns in the community.
              </p>
            </div>
          </CardContent>
        </Card>
