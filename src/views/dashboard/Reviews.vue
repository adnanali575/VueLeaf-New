...
const debouncedFetch = debounce((query: string) => {
  const filters = {
    ...
    sentiment:
      selectedSentiment.value !== "all" ? selectedSentiment.value : undefined,
    platforms:
...
const processContent = (content: string, keyword: string): string => {
  ...
  const stripText = (text: string): string => {
    return text.toLowerCase().replace(/[^a-z0-9]/g, ""); // Remove everything except letters and numbers
  };
  ...
  matches.forEach((match) => {
    if (match.start >= start && match.end <= end) {
      ...
      result += `<span class="bg-green-100 text-green-800">${escapedContent.slice(
        match.start,
        match.end
      )}</span>`;
      lastIndex = match.end;
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 md:py-8">
        ...
        <div class="flex flex-col md:flex-row items-center gap-6">
          <div class="flex-1 text-center md:text-left">
            ...
            <p class="text-gray-600 text-sm sm:text-md md:text-lg">
              Connect directly with your community through thoughtful engagement
              and responses. Our platform helps you manage and reply to
              discussions across the cannabis space, ensuring your brand's voice
              is heard. From answering questions to addressing feedback, build
              stronger relationships with people in real-time. Turn every
              interaction into an opportunity to strengthen your brand's
              reputation.
            </p>
          </div>
          <div class="w-full max-w-xs sm:max-w-sm md:w-48 md:h-48 flex-shrink-0 relative">
            <img :src="analyticsImage" alt="Review Management illustration" class="w-full h-full object-contain" />
          </div>
        </div>
      </div>
      ...
      <div class="mb-8">
        <div class="grid grid-cols-2 grid-rows-2 sm:grid-rows-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div
            class="shadow-sm hover:shadow-md transition-all duration-200 border border-blue-500/10 bg-gradient-to-br from-blue-100/60 to-indigo-100/60 rounded-lg p-6 flex-1">
            ...
            <div class="space-y-4">
              ...
              <div v-else class="text-3xl font-bold text-blue-700">
                {{ reviewsRequiringResponse }}
              </div>
            </div>
          </div>
          ...
          <div
            class="shadow-sm hover:shadow-md transition-all duration-200 border border-emerald-500/10 bg-gradient-to-br from-emerald-100/60 to-green-100/60 rounded-lg p-6 flex-1">
            <div class="flex items-center justify-between mb-6">
            ...
            <div class="space-y-4">
              ...
              <div v-else class="text-3xl font-bold text-emerald-700">
                {{ averageResponseTime }}
              </div>
            </div>
          </div>
          ...
          <!-- Response Rate -->
          <div
            class="shadow-sm hover:shadow-md transition-all duration-200 border border-amber-500/10 bg-gradient-to-br from-amber-100/60 to-yellow-100/60 rounded-lg p-6 flex-1">
            <div class="flex items-center justify-between mb-6">
            ...
            <div class="space-y-4">
              ...
              <div v-else class="text-3xl font-bold text-amber-700">
                {{ responseRate }}%
              </div>
            </div>
          </div>
          ...
          <!-- Flagged Reviews -->
          <div
            class="shadow-sm hover:shadow-md transition-all duration-200 border border-rose-500/10 bg-gradient-to-br from-rose-100/60 to-red-100/60 rounded-lg p-6 flex-1">
            <div class="flex items-center justify-between mb-6">
            ...
            <div class="space-y-4">
              ...
              <div v-else class="text-3xl font-bold text-rose-700">
                {{ flaggedReviews }}
              </div>
            </div>
          </div>
        ...
        <div class="p-6 space-y-4">
          <div>
            ...
            <p class="text-base text-gray-500">
              Filter reviews by platform, keyword, sentiment, and search terms
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="flex flex-col space-y-4">
              <div class="relative">
                ...
                <input v-model="searchQuery" type="text" placeholder="Search reviews..." :disabled="isLoading"
            ...
            <div class="flex flex-col space-y-4">
              <div class="relative">
                ...
                <select v-model="selectedPlatform"
                  ...
                  <option value="thecannacabana.com">The Canna Cabana</option>
      ...
      <div v-else-if="!paginatedReviews.length"
        class="flex flex-col items-center justify-center h-64 bg-white rounded-lg border border-gray-100 p-6 text-center">
        ...
        <p class="text-gray-500 text-sm mb-5 max-w-[700px]">
          Monitor and respond to customer reviews across platforms. Connect with
          your community today.
        </p>
      </div>
      <div v-else class="space-y-6">
        <!-- Reviews List -->
        <div v-for="review in paginatedReviews" :key="review.id" class="relative group">
          <div
            ...
            <div class="flex items-center justify-between flex-wrap gap-3 mb-4">
              ...
              <div class="flex gap-3 flex-wrap sm:flex-nowrap">
                <span
                  ...
                  :class="[
                    getPlatformColor(review.source),
                    'hover:from-opacity-20 hover:to-opacity-15',
                  ]">
                ...
                <span class="inline-flex items-center justify-center px-3 py-1 rounded-md text-sm font-normal"
            ...
            <div>
              <h2 class="text-lg font-medium text-gray-700 mb-4"
                v-html="processContent(review.title, review.keyword_term)"></h2>
              <p class="text-gray-600 leading-relaxed mb-4"
                v-html="processContent(review.content, review.keyword_term)"></p>
            </div>

            <!-- Actions -->
            <div class="flex sm:justify-end flex-wrap gap-2">
              <button @click="openRespondDialog(review)"
              ...
              <button @click="openFlagDialog(review)"
                class="inline-flex items-center px-3 py-1 text-sm font-normal bg-gradient-to-r from-slate-100 to-slate-50 text-slate-700 hover:from-slate-200 hover:to-slate-100 rounded-md transition-all duration-300"
                :class="{ 'from-slate-200 to-slate-100': review.flagged }" :disabled="review.flagged">
                <Flag class="w-4 h-4 mr-1" />
                {{ review.flagged ? "Flagged" : "Flag" }}
              </button>
              <button @click="openHistoryDialog(review)"
                class="inline-flex items-center px-3 py-1 text-sm font-normal bg-gradient-to-r from-slate-100 to-slate-50 text-slate-700 hover:from-slate-200 hover:to-slate-100 rounded-md transition-all duration-300">
      ...
      <!-- Dialogs -->
      <ResponseDialog v-if="selectedReview" :review="selectedReview" :show="showRespondDialog"
        :credentials="credentials" @close="showRespondDialog = false" @submit="handleResponse" />

      <FlagDialog v-if="selectedReview" :review="selectedReview" :show="showFlagDialog" @close="showFlagDialog = false"
        @submit="handleFlag" />

      <!-- History Dialog -->
      <div v-if="showHistoryDialog && selectedReview"
        class="z-[10000] fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
        <div class="bg-white rounded-lg max-w-[425px] w-full">
          <div class="bg-green-700 p-6 rounded-t-lg flex items-center justify-between">
            ...
            <button @click="showHistoryDialog = false"
              class="text-green-100 hover:text-white hover:bg-green-600 p-1 rounded">
              <X class="h-5 w-5" />
            </button>
          </div>
