    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 md:py-8">
        ...
        <div class="flex flex-col md:flex-row items-center gap-6">
          <div class="flex-1 text-center md:text-left">
            ...
            <p class="text-gray-600 text-sm sm:text-md md:text-lg">
              Here you can explore your brand's performance data and metrics through natural conversation. Ask about
              sentiment trends, review patterns, or competitive insights for clear, data-driven answers. Think of me as
              your brand analytics expert, ready to help you understand your impact in the cannabis community today.
            </p>
          </div>
          <div class="w-full max-w-xs sm:max-w-sm md:w-28 md:h-w-28 flex-shrink-0 relative">
            <img :src="teamworkImage" alt="AI Chat Assistant illustration" class="w-full h-full object-contain" />
          </div>
        </div>
      </div>
      ...
      <Card class="mb-8 overflow-hidden">
        ...
        <CardContent class="p-0">
          <div ref="chatContainer" class="h-[80vh] sm:h-[720px] overflow-y-auto p-4 scroll-smooth bg-white">
            <div v-if="!messages.length"
              class="flex flex-col items-center justify-center h-full bg-white p-6 text-center">
                ...
                <div :class="[
                  ...
                  <div v-else>
                    <div class="flex items-center gap-2">
                      <p class="text-[16px] leading-relaxed whitespace-pre-wrap"
                        v-html="message.text.replace(/##LINK\[(.*?)\]##🔗##/g, '<a href=\'$1\' target=\'_blank\'>🔗</a>')">
                      </p>
                      <Loader2 v-if="message.text === '🤖 Robot brain doing backflips...'"
                  ...
                  <div v-if="message.visualization" :key="`viz-container-${message.id}`"
                    class="mt-4 w-[850px] h-[400px] bg-white rounded-lg border border-gray-200 p-4">
                    <KeywordChart :message-id="message.id" :data="message.visualization.data"
                      :type="message.visualization.type" :config="message.visualization.config"
          ...
          <div class="p-4 bg-gray-50 border-t border-gray-200 rounded-b-lg">
            <div class="flex space-x-2">
              <Input v-model="input" @keypress="handleKeyPress" placeholder="Type your message here..."
                class="flex-1 text-[16px] h-11 outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-500"
                :disabled="isLoading" />
              <Button @click="() => wrappedSendMessage(input)" class="bg-green-600 hover:bg-green-700 text-white"
                :disabled="isLoading">
      ...
      <button @click="isQuestionsOpen = true"
      ...
      <!-- Overlay -->
      <div v-if="isQuestionsOpen" class="fixed inset-0 bg-black bg-opacity-50 z-40" @click="isQuestionsOpen = false" />

      <!-- Questions Panel -->
      <div v-if="isQuestionsOpen"
        ...
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-white shadow-sm">
          ...
          <button @click="isQuestionsOpen = false"
        ...
        <!-- Questions Sections -->
        <div ref="questionsPanel" class="h-[calc(100vh-8rem)] overflow-y-auto questions-panel relative"
          @scroll="checkScroll">
          <div class="p-6 space-y-6 pb-32">
            ...
            <div class="space-y-3 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
              <div class="flex items-center gap-2 text-gray-700 bg-gray-50 p-2 rounded-lg">
                <MessageSquare class="w-4 text-green-600" />
                <h3 class="font-medium">Quick Questions</h3>
              </div>
              <div class="space-y-2">
                <Button v-for="question in mainQuestions" :key="question" variant="ghost"
                  class="w-full justify-start h-auto py-2.5 px-4 text-left text-sm hover:bg-gray-50 text-gray-700 hover:text-gray-900"
                  @click="handleQuestionClick(question); isQuestionsOpen = false" :disabled="isLoading">
                  {{ question }}
                </Button>
              </div>
            </div>
            ...
            <!-- Category Questions -->
            <div v-for="catQuestion in categorizedQuestions" :key="catQuestion.category"
              class="space-y-3 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
              <div class="flex items-center gap-2 text-gray-700 bg-gray-50 p-2 rounded-lg">
                <MessageCircle class="w-4 text-green-600" />
                <h3 class="font-medium">{{ catQuestion.category }}</h3>
              </div>
              <div class="space-y-2">
                <Button v-for="question in catQuestion.questions" :key="question" variant="ghost"
          ...
          <div v-if="showScrollIndicator"
            class="fixed bottom-0 right-0 w-96 flex justify-center pointer-events-none z-[60]">
            <div class="bg-gradient-to-t from-[#fafafa] to-transparent h-24 w-full absolute bottom-0"></div>
            <div class="bg-green-200 text-green-900 rounded-full p-2 mb-6 relative shadow-sm flex items-center">
